...
Route::prefix('mi-cuenta')->name('account.')->middleware(['auth', 'customer'])->group(function () {
    ...
    Route::get('/pedidos/{code}', [CustomerOrderController::class, 'show'])
        ->where('code', 'PED-[0-9]{4}-[0-9]{6}')
        ->name('orders.show');
    Route::delete('/pedidos/{code}/cancelar', CustomerOrderCancellationController::class)
        ->where('code', 'PED-[0-9]{4}-[0-9]{6}')
...
Route::prefix('admin')->name('admin.')->group(function () {
    Route::middleware('guest')->group(function () {
        ...
        Route::get('/recuperar-contrasena', [AdminPasswordResetLinkController::class, 'create'])->name('password.request');
        Route::post('/recuperar-contrasena', [AdminPasswordResetLinkController::class, 'store'])
            ->middleware('throttle:3,1')
            ->name('password.email');
        Route::get('/restablecer-contrasena/{token}', [AdminNewPasswordController::class, 'create'])->name('password.reset');
        Route::post('/restablecer-contrasena', [AdminNewPasswordController::class, 'store'])
            ->middleware('throttle:6,1')
            ->name('password.update');
    });
